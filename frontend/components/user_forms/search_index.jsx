...
class SearchIndex extends React.Component {
  ...
  render() {
    const active = this.props.active ? "default-index-search focused-search-result" : "default-index-search";
    let imageurl = this.props.image ? {backgroundImage: `url(${this.props.image})`, content: ' '} : { };
    let icon = this.props.type === 'channel' ?
      ...
      :
      <div className='search-result-icon' style={ imageurl }>
        { this.props.displayName }
      </div>;

    return (
      <div className={ active } ref={ this.props.index } onMouseOver={ this.props.handleHover(this.props.index) }>
        <div className='search-content'>
          <div className='search-result-icon-container'>
            { icon }
          </div>
          <div className='search-result-name'>
            <span className='search-item-name'>{ this.props.name }</span>
          </div>
        </div>
      </div>
    );
