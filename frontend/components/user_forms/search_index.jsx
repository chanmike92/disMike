...
const SearchIndex = (props) => {
    ...
    let imageurl = props.image ? {backgroundImage: `url(${props.image})`, content: ' '} : { };
    let icon = props.type === 'channel' ?
      ...
      // return props.createDm(props.userId).then((payload) => {
      // props.history.push(`/@me/${payload.payload.dm.id}`);
      // props.closeModal();
      // }); }
    return (
      <div className={ active } onMouseOver={ props.handleHover(props.index) } onClick={ (e) => {
          props.handleIndexAction(e); }
           }>
        <div className='search-content'>
          ...
          <div className='search-result-name'>
            <span className='search-item-name'>{ props.name }</span>
          </div>
        </div>
      </div>
    );
